...
//画四个会动的叉叉
void draw_Four_X(u8g2_t *u8g2 ,uint8_t speed)
{


	static u8 y1=0,y2=0,y3=32,y4=32;
	static u8 x1=0,x2=0,x3=64,x4=64;
	u8g2_ClearBuffer(u8g2);
	y1 = (y1+1)%64;
	y2 = (y2+1)%64;
	y3 = (y3+1)%64;
	...
	x3 = (x3+2)%128;
	x4 = (x4+2)%128;
u8g2_DrawLine(u8g2, 0, y1, 127, 63-y1);
u8g2_DrawLine(u8g2, 127, y2, 0, 63-y2);
u8g2_DrawLine(u8g2, 0, y3, 127, 63-y3);
u8g2_DrawLine(u8g2, 127, y4, 0, 63-y4);
	
u8g2_DrawLine(u8g2, x1, 0, 127-x1, 63);
u8g2_DrawLine(u8g2, x2, 63, 127-x2, 0);
u8g2_DrawLine(u8g2, x3, 0, 127-x3, 63);
u8g2_DrawLine(u8g2, x4, 63, 127-x4, 0);
HAL_Delay(256/speed);
}

//绕矩形外边框走的点
void Dot_Move_square(uint8_t *x,uint8_t *y)
{

	static u8 mode = 0;//1加x,2减x,3加y,4减y
	if((*x) == 0 && (*y)<63)
	{
		mode = 3;
	}
...
//绕方框旋转
void draw_JX_Line(u8g2_t *u8g2 ,uint8_t speed)
{


	static u8 y1=0,y2=63 , y3=63,y4=0;
	static u8 x1=0,x2=127 ,x3=0,x4=127 ;
	u8g2_ClearBuffer(u8g2);
	Dot_Move_square(&x1,&y1);
	Dot_Move_square(&x2,&y2);
	Dot_Move_square(&x3,&y3);
	Dot_Move_square(&x4,&y4);
	u8g2_DrawLine(u8g2, x1, y1, x2, y2);
	u8g2_DrawLine(u8g2, x4, y4, x3, y3);
