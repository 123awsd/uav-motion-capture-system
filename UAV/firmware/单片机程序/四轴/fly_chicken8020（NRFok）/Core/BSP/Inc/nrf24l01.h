...
//用户调用函数区
uint8_t NRF24L01_Init(uint8_t mode ,uint8_t ch);//初始化设置模式@NFR_MODE和通信频率(0~40)
void MY_NRF240l_TxPacket_AP(uint8_t *txbuf , uint8_t len);//主接收模式用
uint8_t MY_NRF_Check_RX(uint8_t* rx_buff );
uint8_t NRF24L01_TxPacket(uint8_t *txbuf);                                //主发送模式用
void MY_NRF_TxPacket(uint8_t * tx_buf, uint8_t len);                    //主接收模式用
#endif
